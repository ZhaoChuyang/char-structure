...
from src.preprocess.peanut import PeanutDataset, PeanutClsDataset, OpenPoseDataset
...
def main():
    ...
    with open('data/new_data.json', 'r', encoding='utf8') as fb:
        annotations = json.load(fb)

    if args.model == 'openpose':
        stride = 8
        if args.mode == 'train':
            model = OpenPose('train', 'resnet50', pretrained=False, snapshot='checkpoints/model_99999.pt')
            ...
            dataset = OpenPoseDataset(annotations, images_info, 'data/peanut', stroke_tool=stroke_tool, stride=stride, mode='train')
