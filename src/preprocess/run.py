...
from src.preprocess.peanut import ReinforceDataset, PeanutClsDataset, OpenPoseDataset, Stroke
...
from src.modeling.openpose import OpenPose, SimpleOpenPose, SimpleNet, SimpleClassifier, RefineOpenPose
...
def main():
    ...
    with open('data/refine_train_data.json', 'r', encoding='utf8') as fb:
    ...
    if args.model == 'openpose':
        ...
        if args.mode == 'train':
            if args.reinforcement:
                model = RefineOpenPose('train',
                                       'resnet50',
                                       pretrained=False)
            else:
                model = OpenPose('train',
                                 'resnet50',
                                 pretrained=False)
            ...
            # torch.save(backbone.state_dict(), 'checkpoints/resnet50.pt')

            # model_dict = torch.load('checkpoints/resnet50.pt')
            # model.backbone.load_state_dict(model_dict)
            # print('Backbone checkpoint loaded.')

            # model_dict = torch.load('checkpoints/openpose_model_apex_ep15.pt')['model']
            ...
            if args.reinforcement:
                with open('data/std_skeleton.pkl', 'rb') as fb:
                    std_skeleton = pickle.load(fb)
        ...
        if args.mode == 'test':
            with open('data/data_test.json', 'r', encoding='utf8') as fb:
                annotations = json.load(fb)
            ...
            if args.reinforcement:
                model = RefineOpenPose('test', 'resnet50', pretrained=False)
            else:
            ...
            if args.reinforcement:
                ...
                dataset = OpenPoseDataset(annotations, images_info, 'data/peanut', stroke_tool=stroke_tool, stride=stride,
                                      mode='test', mask=True)
            len_dataset = len(dataset)
            print('loaded %d records' % len_dataset)
