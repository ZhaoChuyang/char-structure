...
from src.modeling.openpose import SimpleNet, OpenPose, RefineOpenPose
import pickle
...
class StructureEstimator(object):
    def __init__(self, cfg):
        ...
        self.model = RefineOpenPose('test', 'resnet50')
        if cfg.snapshot:
    ...
    def test_img(self, image_path, char_id):
        ...
        gt_heatmap = self.skeleton_dict[char_id]['heatmap']
        gt_paf = self.skeleton_dict[char_id]['vecmap']

        stride = self.cfg.stride
        for idx, scale in enumerate(scales):
            input = dict()
            input['img'] = self._get_blob(ori_image, scale)
            input['std_heatmap'] = torch.unsqueeze(gt_heatmap, 0)
            input['std_paf'] = torch.unsqueeze(gt_paf, 0)
            with torch.no_grad():
    ...
    def _extract_paf_info(self, ori_image, paf, all_peaks, char_id=None):
        ...
        """
        image_data = np.asarray(ori_image.convert('L'))
        ...
        for part in range(self.cfg._num_kpts):
            ...
            peaks_binary = np.logical_and.reduce(
                (map_gau >= map_left, map_gau >= map_right, map_gau >= map_up,
                 map_gau >= map_down, map_gau > map_heap/4))
